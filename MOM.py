...
import BACKTEST
from UNIVERSE import ffd_scores                                             # shared trading-prep (was deep_momentum_xgb)
...
hub = DataHub(start="2000-01-01", min_days=0)                               # ONE coherent daily source; MOM resamples to monthly
px, vb, me, m_px = hub.px_d, hub.vol_d, hub.me, hub.m_px
mret = hub.mret.copy()
V = m_px.notna().values; last = len(me) - 1                                 # delist: mark -30% the month after last listing
...
dvd = px * vb; mdv, cov, elig = hub.mdv, hub.cov_m, hub.elig("liquid")       # identical universe (MINPX=5, cov>0.9, MINDV=5e6)
