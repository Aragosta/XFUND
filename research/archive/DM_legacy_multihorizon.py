...
ADDX = int(os.environ.get("ADDX", 0))                                       # 1 = ENGINEERED momentum features (info-discreteness / MAX-lottery / residual-mom)
XF = None
if ADDX:
    print("[MH-DM] building engineered features (infodisc, maxret, resmom) ...", flush=True)
    rd = hub.ret_d.reindex(columns=pm.columns); pdx = hub.px_d.reindex(columns=pm.columns)
    posf = (rd > 0).rolling(252, min_periods=120).mean(); negf = (rd < 0).rolling(252, min_periods=120).mean()
    sgn  = np.sign(pdx / pdx.shift(252) - 1)
    infodisc = sgn * (negf - posf)                                          # Da-Gao-Jagannathan info-discreteness (FiP): <0 = continuous uptrend (stronger momentum)
    maxret   = rd.rolling(21, min_periods=10).max()                         # Bali MAX lottery (negative predictor)
    _mret = rm.mean(axis=1)                                                 # market-beta-neutralized (residual) returns for residual momentum
    _bc = rm.mul(_mret, axis=0).rolling(36, min_periods=24).mean() - rm.rolling(36, min_periods=24).mean().mul(_mret.rolling(36, min_periods=24).mean(), axis=0)
    _bv = (_mret**2).rolling(36, min_periods=24).mean() - _mret.rolling(36, min_periods=24).mean()**2
    _RES = rm.sub((_bc.div(_bv, axis=0).shift(1)).mul(_mret, axis=0))
    resmom = _RES.shift(1).rolling(11, min_periods=8).sum() / (_RES.rolling(11, min_periods=8).std().shift(1) + 1e-9)
    _m = lambda dfd: dfd.reindex(pm.index, method="ffill")                  # daily -> month-end (PIT: last obs at/before month-end)
    XF = {"infodisc": _m(infodisc), "maxret": _m(maxret), "resmom": resmom}
...
def build(H):
    ...
    for t in range(first_feat, T-hmax):
        ...
        if ADDX:
            for nm, df in XF.items(): F[nm] = df.iloc[t-1].reindex(F.index)      # append engineered momentum features
        idx = F.index
