#!/usr/bin/env python3
"""LTR MULTI-OUTPUT: train a t+1 ranker AND a t+2 ranker (same features), combine into one book
score = z(rank_t+1) + z(rank_t+2). Reports t+1, t+2, and COMBINED, plus corr between the two score
...
pool = {}
for k in range(13, T - 2):
    dt = me[k]; P = pd.DataFrame({c: pf[c].loc[dt] for c in PF})
    l1, l2, pnl = mret.iloc[k + 1], mret.iloc[k + 2], mret.iloc[k + 1]
...
print("[fit] t+1 & t+2 rankers ...", flush=True)
s1, s2 = {}, {}; st1 = st2 = None
...
print("\n" + "=" * 66)
print("=== LTR multi-output: t+1, t+2, COMBINED (daily 750, gross) ===")
print(f"{'signal':22}{'ann':>8}{'sharpe':>8}{'maxDD':>9}{'ann.turn':>10}")
for nm, a, sh, m, t in [("t+1", a1, sh1, m1, t1), ("t+2", a2, sh2, m2, t2), ("COMBINED z(t+1)+z(t+2)", ac, shc, mc, tc)]:
    print(f"{nm:22}{a:>8.1%}{sh:>8.2f}{m:>9.1%}{t:>10.0%}")
print(f"\navg cross-sectional corr(t+1 score, t+2 score) = {sc_corr:.2f}")
print(f"corr(t+1 returns, t+2 returns) = {pd.DataFrame({'a':r1,'b':r2}).dropna().corr().iloc[0,1]:.2f}")
